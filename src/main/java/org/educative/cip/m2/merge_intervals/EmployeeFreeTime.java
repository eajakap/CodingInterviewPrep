...
public class EmployeeFreeTime {
    ...
    static class Solution {
        ...
        public static List<Interval> employeeFreeTime(List<List<Interval>> schedule) {
            ...
            // Use a custom comparator to sort the heap based on the start time of the intervals.
            PriorityQueue<int[]> heap = new PriorityQueue<>((a, b) -> a[0] - b[0]);

            // Iterate for all employees' schedules
            // and add start of each schedule's first interval along with
            // its index value and a value 0.
            // index value (i) is used to identify which employee's schedule the interval belongs to.
            for (int i = 0; i < schedule.size(); i++) {
                List<Interval> employeeSchedule = schedule.get(i);
                Interval interval = employeeSchedule.get(0);
                // Add the start of the interval, employee index, and the index of the interval (0) into the heap.
            ...
            // The first interval in the heap is the one with the earliest start time.
            int previous = schedule.get(heap.peek()[1]).get(heap.peek()[2]).end;
            ...
            while (!heap.isEmpty()) {
                ...
                if (j + 1 < schedule.get(i).size()) {
                    ...
                    // Push the next interval's start value, employee index, and the next interval's index into the heap.
